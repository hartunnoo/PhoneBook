...
public class AiController : ControllerBase
{
    ...
    [HttpGet("chat")]
    public async Task<IActionResult> Chat([FromQuery] string q, CancellationToken ct = default)
    {
        if (string.IsNullOrWhiteSpace(q)) return BadRequest(new { error = "Query required" });
        var all = await _contacts.GetAllAsync(null, ct);
        var results = all.Take(100).ToList();
        return Ok(new { explanation = "Hasil carian untuk: " + q, total = results.Count, isGeneral = true,
